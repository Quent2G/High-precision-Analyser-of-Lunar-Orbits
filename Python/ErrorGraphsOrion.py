...
Hrz = read_horizon("input/CAPSTONE26_30.txt")
...
#plot traj
ax.plot(Hrz["X"],Hrz["Y"],Hrz["Z"],"r",zorder=10,label="traj")
ax.set_xlabel('X (km)')
ax.set_ylabel('Y (km)')
ax.set_zlabel('Z (km)')
...
state = Hrz.loc[0]
ax.plot(state["X"],state["Y"],state["Z"],"b.",zorder=10,label="Initial pos")        #Starting point

# Plot final position
state = Hrz.loc[len(Hrz)-1]
ax.plot(state["X"],state["Y"],state["Z"],"c.",zorder=10,label="Final Pos True")        #Ending point True

# Plot final position Precise Prop
EndP = XJ2000[-1]
ax.plot(EndP[0],EndP[1],EndP[2],"y.",zorder=10,label="Final Pos Prop")        #Starting point All
ax.legend()
plt.axis('equal')
###
